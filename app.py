...
datapath = os.getenv('SIERRA_DATA_PATH')
if not datapath:
...
for basin in ['stn', 'tuo', 'mer', 'usj']:
    ...
    with open('../sierra-pywr/pywr-models/models/{}/pywr_model.json'.format(basin_long)) as f:
        m = json.load(f)
    ...
    filepath = opath.format(basin=basin_long.replace('_', ' ').title() + ' River', attr='streamflow_cfs')
    df = pd.read_csv(filepath, index_col=[0], parse_dates=True).ffill()  # already cfs, no need to concert
...
def update_selected_resources(url, basin):
    ...
    root_dir = constants.DEV_RESULTS_PATH
    results_path = os.path.join(root_dir, 'development/{basin}/historical/Livneh'.format(basin=basin_name))
    tabs = []
